    ...
    for cluster_num in REGION_CLUSTER_ORDER[REGION]:
        ...
        with col2:
            ...
            if family_sales.height > 0:
                ...
                fig.update_layout(height=250, margin=dict(l=0, r=40, t=40, b=60),
...
with tab_holiday:
    ...
    with h_col1:
        ...
        h_holiday = st.selectbox("휴일 선택", holiday_opts, key='holiday_select_s')
    with h_col2:
        family_list = get_family_list(REGION)
        h_family = st.selectbox("카테고리 선택", ['전체'] + family_list, key='family_select_s')
