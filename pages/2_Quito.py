...
with tab_total:
    ...
    for cluster_num in cluster_order:
        ...
        # 데이터 캐싱 (필터 조합별로)
        total_cache_key = f"total_data_{REGION}_{cluster_num}_{year_filter}_{quarter_filter}_{month_filter}"
        if total_cache_key not in st.session_state:
            with st.spinner('데이터 로딩 중...'):
                st.session_state[total_cache_key] = {
        ...
        train = st.session_state[total_cache_key]['train']
        trans = st.session_state[total_cache_key]['trans']
        ...
        with col1:
            ...
            if train.height > 0:
                ...
                fig = go.Figure()
                fig.add_trace(go.Bar(x=sales_daily['date'], y=sales_daily['unit_sales'],
            ...
            if trans.height > 0:
                ...
                fig = go.Figure()
                fig.add_trace(go.Bar(x=trans_daily['date'], y=trans_daily['transactions'],
...
# ============================================================
with tab_base:
    # 세션 상태에 데이터 캐싱 (같은 클러스터면 재로드 방지)
    cache_key = f"base_data_{REGION}_{selected_cluster}"
    if cache_key not in st.session_state:
        with st.spinner("데이터 로딩 중..."):
            try:
                ...
                st.session_state[cache_key] = {'loaded': False, 'error': str(e)}
    
    cached = st.session_state[cache_key]
    base_loaded = cached.get('loaded', False)
    if base_loaded:
        train_data = cached['train_data']
        predictions = cached['predictions']
        family_totals = cached['family_totals']
    else:
        st.error(f"데이터 로드 실패: {cached.get('error', '알 수 없는 오류')}")
    ...
    if base_loaded:
        ...
        with col_reset:
            st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True)
            if st.button('필터 초기화', key='reset_base'):
                st.session_state.reset_base_slider = True
                st.rerun()
        
        train_filtered = train_data.filter(
            (pl.col("DATE") >= selected_dates[0]) & (pl.col("DATE") <= selected_dates[1])
        )
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=train_daily['DATE'], y=train_daily['TOTAL_SALES'],
                                mode='lines', name='Train', line=dict(color=COLORS['train_line'], width=2)))
        fig.add_trace(go.Scatter(x=pred_daily['DATE'], y=pred_daily['PRED_SALES'],
                                mode='lines', name='Test (예측)', line=dict(color=COLORS['test_line'], width=2)))
        ...
        st.markdown("---")
        st.markdown("### 상위 Family 판매량 실적·예측")
        
        top3_families = family_totals.head(3).select("FAMILY").to_series().to_list()
        selected_top = st.selectbox("", options=top3_families, index=0, key="top_family_quito")
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=train_top['DATE'], y=train_top['TOTAL_SALES'], mode='lines', name='Train', line=dict(color=COLORS['train_line'], width=2)))
        fig.add_trace(go.Scatter(x=pred_top['DATE'], y=pred_top['PRED_SALES'], mode='lines', name='Test', line=dict(color=COLORS['test_line'], width=2)))
        fig.add_vline(x="2016-08-16", line_width=2, line_dash="dash", line_color="#E31837")
        fig.update_layout(**get_chart_layout(""))
        fig.update_layout(
            height=400,
        ...
        fig.update_xaxes(title_text="")
        fig.update_yaxes(title_text="")
        fig.update_layout(xaxis=dict(range=["2015-08-16", "2017-08-15"], fixedrange=True), yaxis=dict(fixedrange=True))
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
        
        st.markdown("---")
        st.markdown("### 하위 Family 판매량 실적·예측")
        
        bottom3_families = family_totals.tail(3).select("FAMILY").to_series().to_list()
        selected_bottom = st.selectbox("", options=bottom3_families, index=0, key="bottom_family_quito")
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=train_bottom['DATE'], y=train_bottom['TOTAL_SALES'], mode='lines', name='Train', line=dict(color=COLORS['train_line'], width=2)))
        fig.add_trace(go.Scatter(x=pred_bottom['DATE'], y=pred_bottom['PRED_SALES'], mode='lines', name='Test', line=dict(color=COLORS['test_line'], width=2)))
        fig.add_vline(x="2016-08-16", line_width=2, line_dash="dash", line_color="#E31837")
        fig.update_layout(**get_chart_layout(""))
        fig.update_layout(
            height=400,
...
# ============================================================
with tab_holiday:
    # 클러스터링 데이터는 TOTAL 탭에서 이미 로드됨 - 재사용
    clustering_df_h = clustering_df
    ...
    with f_col1:
        ...
        h_holiday = st.radio("축제", options=holiday_opts,
                            ...
                            key='h_holiday_quito', label_visibility='collapsed',
                            horizontal=True)
    ...
    with f_col2:
        ...
        h_family = st.selectbox('카테고리', family_opts, key='h_family_quito', label_visibility='collapsed')
    
    with f_col3:
        st.markdown("#### 연도")
        h_year = st.selectbox('연도', ['전체', '2014', '2015', '2016'], key='h_year_quito', label_visibility='collapsed')
    
    st.markdown("---")
    
    # 데이터 로드 (캐싱 적용)
    h_stores = clustering_df_h.filter(pl.col('cluster') == selected_cluster).select('store_nbr').to_series().to_list()
    
    holiday_cache_key = f"holiday_data_{REGION}_{selected_cluster}_{h_year}"
    if holiday_cache_key not in st.session_state:
        with st.spinner('데이터 분석 중...'):
            st.session_state[holiday_cache_key] = load_train_data_for_holiday(h_stores, h_year)
    train_h = st.session_state[holiday_cache_key]
    
    # 프로모션/예측 데이터 로드
    promotion_df = load_promotion_timing_predictions(REGION, cluster_key, h_holiday, h_family if h_family != '전체' else None)
    forecast_df = load_sales_forecast_predictions(cluster_key, model_type)
