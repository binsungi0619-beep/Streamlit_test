...
with tab_base:
    ...
    if base_loaded:
        ...
        selected_top = st.selectbox("", options=top3, index=0, key="top_family_c")
        ...
        selected_bottom = st.selectbox("", options=bottom3, index=0, key="bottom_family_c")
...
with tab_holiday:
    ...
    with f_col1:
        ...
        h_holiday = st.radio("축제", options=holiday_opts,
                            ...
                            key='h_holiday_costa', label_visibility='collapsed',
                            horizontal=True)
    ...
    with f_col2:
        ...
        h_family = st.selectbox('카테고리', family_opts, key='h_family_costa', label_visibility='collapsed')
    
    with f_col3:
        st.markdown("#### 연도")
        h_year = st.selectbox('연도', ['전체', '2014', '2015', '2016'], key='h_year_costa', label_visibility='collapsed')
    
    st.markdown("---")
    
    # 데이터 로드 (전체 매장 대상) - 캐싱 적용
    h_stores = clustering_df_h.select('store_nbr').to_series().to_list()
    
    holiday_cache_key = f"holiday_data_{REGION}_{h_year}"
